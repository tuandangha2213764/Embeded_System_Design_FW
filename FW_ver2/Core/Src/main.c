...
void Update_Setpoint_Logic(void) {
    ...
    // Xử lý Input Type (Setpoint/Sine/Pulse)
    if (input_type == 0) { 
        // Giữ nguyên set_point từ UART
    } 
    else if (input_type == 1) {
        set_point = set_point_sine; // Biến sine từ Simulink
    } 
    else if (input_type == 2) {
        static uint16_t pulse_counter = 0;
        
        // Logic tạo xung vuông
...
void Send_Telemetry_Data(void) {
    if (uart_flag_tx) {
        if (controller_type == 0) {
            ...
            send_uart_3float(&huart1, recovered_speedmras, current_speed, signal_controlPImras);
        }
        uart_flag_tx = 0;
    }
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // --- KHỞI TẠO HỆ THỐNG (ĐÃ SỬA LỖI LẶP) ---
